import { db, collection, doc, getDoc, addDoc, updateDoc, getDocs, query, where } from './firebase-config.js';
import { escapeHtml, formatDate, showToast, checkAuth, converterLinkGoogleDrive } from './utils.js';
...
window.submitQuiz = async () => {
  ...
  if (passed) {
    ...
    try {
      ...
      // Atualizar atribuição no Firebase (se existir)
      const atribQ = query(collection(db, 'atribuicoes'), 
        where('colaboradorUser', '==', nomeUser),
        where('cursoId', '==', cursoId),
        where('status', '==', 'pendente')
      );
      const atribSnap = await getDocs(atribQ);
      atribSnap.forEach(async (docAtrib) => {
        await updateDoc(doc(db, 'atribuicoes', docAtrib.id), {
          status: 'concluido',
...
async function showCertificate() {
  ...
  const certHtml = `
    <div class="cert-screen" style="display:block; margin-top: 30px;" id="cert-screen">
